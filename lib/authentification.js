...
authentification.prototype.shortTerm = function(msg, cb) {
  ...
  if (!username || !msg.getAttribute('message-integrity')) {
    ...
    if (password && this.checkIntegrity(msg, password)) {
...
authentification.prototype.longTerm = function(msg, cb) {
  ...
  var hmacKey = crypto.createHash('md5').update(username + ':' + this.server.realm  + ':' + password).digest();
  if (!this.checkIntegrity(msg, hmacKey)) {
    // generate an error response with an error code of 401 (Unauthorized).
  ...
  var user = new User(username, hmacKey);
...
authentification.prototype.checkIntegrity = function(msg, hmacKey) {
  var messageIntegrity = msg.getAttribute('message-integrity');
  var hmac = crypto.createHmac('sha1', hmacKey).update(msg.hmacInput).digest('hex');
  return (hmac === messageIntegrity);
};
